patient_journey.uro <- swimmer.urology <- function(patient, x_min = 6) {
  #patient = 'D501C5AF-872E-4DBF-8340-628128ECFD93'
  # patient='FC9F9140-5D3B-4124-8A7B-701664D60EC7'
  ...
  events.urology <- index_date.data %>%
    inner_join(encounters %>% filter(grepl('urology', specialty, ignore.case = T)),
               by = c('patientid')) %>%
    convert_dates() %>%
    mutate(
      event_date = startdts,
